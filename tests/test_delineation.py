...
def main() -> int:
    ...
    check("Saturn's 1st house depends on the Lagna sign",
          ...
          and d.planet_house_text("Saturn", 1, "Leo") == d._SATURN_HOUSE1_ORDINARY)
    ...
    print("\n2b. Bhrigu per-Lagna house table — Aries, Taurus, Gemini, Cancer so far")
    ...
    check("a Lagna not yet transcribed (e.g. Leo) falls back cleanly, no KeyError",
          d.planet_house_text("Sun", 1, "Leo") == d._BASE_HOUSE_TEXT[1])
    ...
    check("Rahu/Ketu are still refused without a covering Lagna, not guessed at",
          _raises(lambda: d.planet_house_text("Rahu", 5, "Leo"), KeyError)
