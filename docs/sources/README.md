...
- **`bhrigu_samhita_notes.md`** — a modern (1975) Hindi compilation, *Bhrigu
  ...
  Gemini (Mithuna), Cancer (Karka), Leo (Simha), Virgo (Kanya) and Libra
  (Tula) are done; the notes file tracks which Lagna is next and two
  documented per-Lagna gaps (a real scan gap for Aries, an internally
  contradictory source entry for Taurus — Gemini, Cancer, Leo, Virgo and
  Libra have none).
