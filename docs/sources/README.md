...
- **`bhrigu_samhita_notes.md`** — a modern (1975) Hindi compilation, *Bhrigu
  ...
  (Tula), Scorpio (Vrishchika), Sagittarius (Dhanu) and Capricorn (Makara)
  are done; the notes file tracks which Lagna is next and two documented
  per-Lagna gaps (a real scan gap for Aries, an internally contradictory
  source entry for Taurus — Gemini, Cancer, Leo, Virgo, Libra, Scorpio,
  Sagittarius and Capricorn have none).
