...
- **`bhrigu_samhita_notes.md`** — a modern (1975) Hindi compilation, *Bhrigu
  ...
  Gemini (Mithuna), Cancer (Karka), Leo (Simha) and Virgo (Kanya) are done;
  the notes file tracks which Lagna is next and two documented per-Lagna
  gaps (a real scan gap for Aries, an internally contradictory source
  entry for Taurus — Gemini, Cancer, Leo and Virgo have none).
